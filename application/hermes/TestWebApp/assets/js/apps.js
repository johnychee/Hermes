...
var handleSidebarMinify = function() {
    $('[data-click=sidebar-minify]').click(function(e) {
        ...
        var targetContainer = '#page-container';
        /* this shit added */
        var bgClass = 'bg-minified';
        var bgContainer = '.sidebar-bg';
        /* until here */
        if ($(targetContainer).hasClass(sidebarClass)) {
            $(targetContainer).removeClass(sidebarClass);
            $(bgContainer).removeClass(bgClass);/* this shit added */
            if ($(targetContainer).hasClass('page-sidebar-fixed')) {
                generateSlimScroll($('#sidebar [data-scrollbar="true"]'));
            }
        } else {
            $(targetContainer).addClass(sidebarClass);
            $(bgContainer).add(bgClass);/* this shit added */
            if ($(targetContainer).hasClass('page-sidebar-fixed')) {
...
/* Notification Reset After Click */
var notificationResetAfterClick = function (userId) {
    $('#notification a[data-toggle=dropdown]').click(function () {
        // set as readed
        $.getJSON("/Notification/SetAllReaded/" + userId);
        // update icon
        $('#notification a span.label').addClass('bg-green-lighter').html('0');
    });
};

/* auto-refresh - 1min */
var notificationAutoRefresh = function (userId) {
    setInterval(function () {
        $.getJSON("/Notification/getNew/" + userId)
          .error(function () {
              console.log("Could not connect to '/Notification/getNew/" + userId + "'");
          })
          .success(function (data) {
              var notifArea = $('#notification ul.dropdown-menu');
              $.each(data, function (i, notification) {
                  if (notifArea.find('input[value=' + notification['Id'] + ']').size() == 0) {
                      var link = notification['ticketId'] == null ? "javascript:;" : "/Ticket/Details/" + notification['ticketId'];
                      var typeBackground;
                      var typeIcon;
                      switch (notification['NotificationType']) {
                          case "Emailing":
                              typeBackground = "bg-green-lighter";
                              typeIcon = "fa fa-envelope";
                              break;
                          case "SLA":
                              typeBackground = "bg-red";
                              typeIcon = "fa fa-bolt";
                              break;
                          case "System":
                              typeBackground = "bg-aqua-darker";
                              typeIcon = "fa fa-info";
                              break;
                          case "TicketFlow":
                              typeBackground = "bg-yellow-darker";
                              typeIcon = "fa fa-flag";
                              break;
                      }
                      // set red bg
                      $('#notification span.label').removeClass('bg-green-lighter');
                      // add dropdown
                      notifArea.find('li.dropdown-header').after('<li class="media"><a href="' + link + '"><div class="pull-left media-object ' + typeBackground + '"><i class="' + typeIcon + '"></i></div><div class="media-body"><h6 class="media-heading">' + notification['Subject'] + '</h6><div class="text-muted">' + notification['CreatedAgo'] + '</div><input type="hidden" name="id" value="' + notification['Id'] + '" /></div></a></li>');
                      // set count
                      $('#notification span.label').html(notifArea.find('li.media').size());
                  }
              });
          });
    }, 60000);
...
var App = function () {
	...
	return {
		...
		init: function (userId) {
		    ...
		    // notifications
			notificationResetAfterClick(userId);
			notificationAutoRefresh(userId);
		}
  };
}();