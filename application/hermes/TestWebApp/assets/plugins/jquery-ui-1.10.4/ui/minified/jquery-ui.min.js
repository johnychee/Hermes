...
var OrderedHash={d:{},o:{},l:0,add:function(k,v){this.d[k]=[this.l,v];this.o[this.l]=k;this.l++;return this;},addFromArray:function(a){for(i in a){this.add(a[i][0],a[i][1]);}},hasKey:function(k){return typeof this.d[k]!=='undefined';},get:function(k){return this.d[k][1];},removeByKey:function(k){var i=this.d[k][0];delete this.d[k];for(y=i+1;y<this.l;y++){this.o[y-1]=this.o[y];}this.l--;return this;},each:function(c){for(i=0;i<this.l;i++){var k=this.o[i];c(k,this.d[k][1]);}return this;}};
var singleValue={k:null,v:null,l:1,add:function(k,v){this.k = k;this.v=v;},addFromArray:function(a){for(i in a){this.add(a[i][0],a[i][1]);}},hasKey:function(k){return this.k==k;},get:function(k){if(this.k==k)return this.v;return null;},removeByKey:function(k){if(this.k==k)this.v=null;return this;},each:function(c){c(this.k,this.v);return this;}};
jQuery.fn.extend({modalSelection:function(a){var o=$(this);if(typeof a['resultTitle'] === 'undefined')a['resultTitle']='Users';if(typeof a['resultName']==='undefined')a['resultName']='result';if(typeof a['multipleSelect']==='undefined')a['multipleSelect']=false;if(a['multipleSelect'])var s=Object.create(OrderedHash);else var s=Object.create(singleValue);if(typeof a['startValue']!=='undefined')s.addFromArray(a['startValue']);var safeUrl=function(d){return d.replace(/ /g,'')};var getUsers=function(){var u=a['url'];if(typeof a['filters']!=='undefined'){u+='?';$.each(a['filters'],function(n){u+=safeUrl(n)+"="+$('#modalSelection div[data-contains='+safeUrl(n)+'] ul li.selected').attr('data-id')+'&';});}$.getJSON(u).error(function(){modalError('cannot connect to server');}).success(function(d){var t="";$.each(d, function(i,b){t+='<li class="list-group-item'+(s.hasKey(b['id']) ?" selected":"")+'">'+b['name']+'<input type="hidden" value="'+b['id']+'"></li>';});$('#modalSelection div[data-contains=result] ul').html(t);setEvent_resultListClick('#modalSelection div[data-contains=result] ul .list-group-item');});};var writeUsersToPage=function(){var h='';if(a['multipleSelect']){s.each(function(i,b){h+='<div class="list-group-item">'+b+'<input type="hidden" name="'+a['resultName']+'_'+i+'" value="'+i+'" /></div>';})}else{s.each(function(i,b){h=b+'<input type="hidden" name="'+a['resultName']+'" value="'+i+'">';});}o.find('.selected_items').html(h);};var setEvent_resultListClick=function(e){$(e).click(function(){if($(this).hasClass('selected')){if(a['multipleSelect']){$(this).removeClass('selected');s.removeByKey($(this).find('input').val());}}else{if(!a['multipleSelect']){var p=$(this).parent().find('.selected');p.removeClass('selected');s.removeByKey(p.find('input').val());}$(this).addClass('selected');s.add($(this).find('input').val(),$(this).text());}});};var setEvent_filterListClick=function(e){$(e).click(function(){$(this).parent().children().removeClass('selected');$(this).addClass('selected');getUsers();});};var modalError=function(m){$('#modalSelection .alert-danger').html(m+'<span class="close">X</span>').click(function(){$(this).parent().addClass('hidden');}).parent().removeClass('hidden');};if(a['multipleSelect']){var p='<a class="btn btn-primary" data-toggle="modal" data-target="#modalSelection">Select</a><div class="list-group list-group-minor selected_items"></div>'}else{var p='<span class="selected_items"></span><a class="btn btn-primary" data-toggle="modal" data-target="#modalSelection">Select</a>';}$(this).html(p);writeUsersToPage();if($('#modalSelection').size()<1){var m='<div class="modal fade" id="modalSelection"><div class="modal-dialog"><div class="modal-content"><div class="modal-header"><button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button><h4 class="modal-title">Set Customer</h4></div><div class="modal-body row"><div class="col-md-12 hidden"><div class="alert alert-danger fade in m-b-15"></div></div>';$.each(a['filters'],function(b,f){m+='<div class="col-md-4 col-xs-12" data-contains="'+safeUrl(b)+'"><h4>'+b+'</h4><ul class="list-group"><li class="list-group-item selected">All</li>';$.each(f,function(i,v){m+='<li class="list-group-item" data-id="'+v['id']+'">'+v['name']+'</li>';});m+='</ul></div>';});m+='<div class="col-md-4 col-xs-12" data-contains="result"><h4>'+a['resultTitle']+'</h4><ul class="list-group"></ul></div></div><div class="modal-footer"><button type="button" class="btn btn-primary" data-action="accept">Select</button><button type="button" class="btn btn-default" data-dismiss="modal">Close</button></div></div></div></div>';$('body').append(m);setEvent_filterListClick('#modalSelection ul .list-group-item');getUsers();$('#modalSelection').on('shown.bs.modal');$('#modalSelection button[data-action=accept]').click(function(){writeUsersToPage();$('#modalSelection').modal('hide');});}}});}(jQuery);
